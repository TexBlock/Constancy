...
public abstract class ForgifiedFabricApiExtension {
    ...
    private String getDependencyNotation(String moduleName, String fabricApiVersion) {
        return String.format("dev.su5ed.sinytra.fabric-api:%s:%s", moduleName, moduleVersion(moduleName, fabricApiVersion));
    }
    ...
    private File getPom(String name, String version) throws PomNotFoundException {
        ...
        try {
            if (this.isSinytraVersion == true) {
                extension.download(String.format("https://maven.su5ed.dev/releases/dev/su5ed/sinytra/fabric-api/%2$s/%1$s/%2$s-%1$s.pom", version, name))
                        .defaultCache()
                        .downloadPath(mavenPom.toPath());
            } else {
                extension.download(String.format("https://maven.kessokuteatime.work/snapshots/dev/su5ed/sinytra/fabric-api/%2$s/%1$s/%2$s-%1$s.pom", version, name))
                        .defaultCache()
                        .downloadPath(mavenPom.toPath());
            }
